...
CMS_WRITE_PERMS = ("website_cms.view", "view-website_cms", "settings.view", "view-settings", "update-settings")
...
def create_banner(data: BannerPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_banner(db, data)}

@router.put("/homepage-banners/{item_id}")
def update_banner(item_id: int, data: BannerPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_banner(db, item_id, data)}

@router.delete("/homepage-banners/{item_id}")
def delete_banner(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
def create_destination(data: PopularDestinationPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_destination(db, data)}

@router.put("/popular-destinations/{item_id}")
def update_destination(item_id: int, data: PopularDestinationPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_destination(db, item_id, data)}

@router.delete("/popular-destinations/{item_id}")
def delete_destination(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
def create_popular_tour(data: PopularTourPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_popular_tour(db, data)}

@router.delete("/popular-tours/{item_id}")
def delete_popular_tour(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
def create_deal(data: TourOnDealPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_deal(db, data)}

@router.put("/tours-on-deals/{item_id}")
def update_deal(item_id: int, data: TourOnDealPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_deal(db, item_id, data)}

@router.delete("/tours-on-deals/{item_id}")
def delete_deal(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
@router.post("/blogs")
def create_blog(data: BlogPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_blog(db, data)}

@router.put("/blogs/{item_id}")
def update_blog(item_id: int, data: BlogPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_blog(db, item_id, data)}

@router.delete("/blogs/{item_id}")
def delete_blog(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
def create_review(data: ReviewPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_review(db, data)}

@router.put("/customer-reviews/{item_id}")
def update_review(item_id: int, data: ReviewPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_review(db, item_id, data)}

@router.delete("/customer-reviews/{item_id}")
def delete_review(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
def create_help(data: HelpArticlePayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_help(db, data)}

@router.put("/help-centre/{item_id}")
def update_help(item_id: int, data: HelpArticlePayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_help(db, item_id, data)}

@router.delete("/help-centre/{item_id}")
def delete_help(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
@router.put("/policies")
def upsert_policy(data: PolicyPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
def create_popup(data: PopupPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_popup(db, data)}

@router.put("/promotional-popups/{item_id}")
def update_popup(item_id: int, data: PopupPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_popup(db, item_id, data)}

@router.delete("/promotional-popups/{item_id}")
def delete_popup(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
def create_link(data: ExternalLinkPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_external_link(db, data)}

@router.put("/external-links/{item_id}")
def update_link(item_id: int, data: ExternalLinkPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_external_link(db, item_id, data)}

@router.delete("/external-links/{item_id}")
def delete_link(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
...
@router.post("/sitemap")
def create_sitemap_entry(data: SitemapEntryPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.create_sitemap_entry(db, data)}

@router.put("/sitemap/{item_id}")
def update_sitemap_entry(item_id: int, data: SitemapEntryPayload, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):
    return {"status": "success", "data": service.update_sitemap_entry(db, item_id, data)}

@router.delete("/sitemap/{item_id}")
def delete_sitemap_entry(item_id: int, db: Session = Depends(get_db), _=Depends(require_any_permission(*CMS_WRITE_PERMS))):

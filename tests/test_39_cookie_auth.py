...
from app.routers.auth import ACCESS_COOKIE_NAME, CSRF_COOKIE_NAME, REFRESH_COOKIE_NAME, _clear_auth_cookies, _set_auth_cookies
...
def test_auth_cookies_are_httponly_and_refresh_token_is_not_serialized():
    ...
    assert any(cookie.startswith(f"{CSRF_COOKIE_NAME}=") and "HttpOnly" not in cookie for cookie in cookies)
...
def test_logout_expires_both_auth_cookies():
    ...
    assert any(cookie.startswith(f'{CSRF_COOKIE_NAME}=""') and "Max-Age=0" in cookie for cookie in cookies)
